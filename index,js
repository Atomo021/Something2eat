...
const GLUTTON_AGENT_PERCENTAGE = 0.25;
...
const DRUNK_AGENT_PERCENTAGE = 0.10; // 10% de probabilidad de ser un borracho
...
function Agent(id, startX, startY) {
    ...
    this.isDrunk = Math.random() < DRUNK_AGENT_PERCENTAGE; // NEW: Propiedad para borracho
    ...
    // Lógica para asignar la capacidad de aprendizaje por esfuerzo
    if (this.isDrunk) { // Un agente borracho no puede aprender ninguna habilidad por esfuerzo
        this.canLearnSwimThroughEffort = false;
        this.canLearnClimbThroughEffort = false;
    } else if (!this.isLazy) { // Lógica de aprendizaje normal para agentes no flojos y no borrachos
        if (this.isProdigy) {
            this.canLearnSwimThroughEffort = !this.canSwim;
            this.canLearnClimbThroughEffort = !this.canClimb;
        } else {
            const availableSkillsToLearn = [];
            if (!this.canSwim) {
                availableSkillsToLearn.push('swim');
            }
            if (!this.canClimb) {
                availableSkillsToLearn.push('climb');
            }

            if (availableSkillsToLearn.length > 0) {
                const skillToAssign = availableSkillsToLearn[Math.floor(Math.random() * availableSkillsToLearn.length)];
                if (skillToAssign === 'swim') {
    ...
    } else { // Agente flojo y no borracho, no puede aprender
        this.canLearnSwimThroughEffort = false;
        this.canLearnClimbThroughEffort = false;
    }
    ...
    this.move = function() {
                        ...
                        if (!isStayingPut && !this.isLazy) {
                            ...
                            if (cellContent === 'm' && !this.canClimb && this.canLearnClimbThroughEffort) {
                                ...
                                if (this.climbEffort >= LEARNING_CLIMB_THRESHOLD) {
                                    this.canClimb = true;
                                    this.climbEffort = 0;
                                    logMessage(`¡Agente ${this.id} aprendió a escalar por esfuerzo!`, '#00FF00');
                                }
                                ...
                                if (this.swimEffort >= LEARNING_SWIM_THRESHOLD) {
                                    this.canSwim = true;
                                    this.swimEffort = 0;
                                    logMessage(`¡Agente ${this.id} aprendió a nadar por esfuerzo!`, '#00FF00');
                                }
                            }
                        }
                    }
            ...
            if (intendedX !== currentX || intendedY !== currentY) { // Si intenta moverse a una nueva celda
                ...
                if (Array.isArray(world[intendedY]) && typeof world[intendedY][intendedX] !== 'undefined') {
                    ...
                    if (cellContentAtIntended === 'a') { // Intentando moverse al agua
                        ...
                        logMessage(`Agente Borracho ${this.id} intentó entrar al agua (${this.waterAttemptCounter}/15)`, 'grey');
                        if (this.waterAttemptCounter >= 15) {
                            this.isAlive = false;
                            logMessage(`¡Agente Borracho ${this.id} se ahogó en el agua!`, 'red');
                            stopTimer(); 
                        }
                        ...
                        logMessage(`Agente Borracho ${this.id} intentó escalar la montaña (${this.consecutiveMountainAttempts}/10)`, 'grey');
                        if (this.consecutiveMountainAttempts >= 10) {
                            logMessage(`¡Agente Borracho ${this.id} se tropezó por la montaña tras insistir!`, 'orange');
        ...
        if (!this.isDrunk || !wasDrunkObstacleAttempt) { // Si no es borracho, o si es borracho pero no fue bloqueado por una regla de obstáculo específico de borracho
            if (finalX === currentX && finalY === currentY) {
                ...
                if (this.consecutiveSameTargetMoves >= STUCK_THRESHOLD) {
                    logMessage(`Agente ${this.id} se siente atascado moviéndose repetidamente a (${finalX},${finalY}). Busca nueva ruta.`, 'orange');
        ...
        if (this.energy <= 0) {
            ...
            logMessage(`¡Agente ${this.id} ha muerto! ¡Conocimiento completamente perdido!`, 'red');
        }
    };
    ...
    this.act = function() {
        ...
        if (world[this.y][this.x] === 'C') {
            ...
            logMessage(`Agente ${this.id} comió comida! Energía: ${this.energy} (Comida restante: ${totalFoodCount})`, '#00FFFF');
        }
    };
}
    ...
    try { 
        agents.forEach(agent => {
            if (agent.isAlive && (totalFoodCount > 0 || simulationInterval !== null)) {
                try { 
                    ...
                    console.error(`ERROR EN AGENTE ${agent.id}:`, agentError);
                    agent.isAlive = false; 
                    logMessage(`¡Agente ${agent.id} ha muerto por un error inesperado!`, 'red');
                    stopTimer(); 
        ...
        if (agents.length > 0) {
            ...
            if (agent.isAlive) {
                ...
                statusElement.textContent = `Agente 1: Muerto`;
        ...
        if (totalFoodCount === 0 && simulationInterval !== null) {
            ...
            const gridElement = document.getElementById('game-grid');
            gridElement.innerHTML = `<p style="font-size: 1.5em; color: yellow; text-align: center;">¡FIN DE LA PARTIDA!</p><p style="font-size: 1em; color: white; text-align: center;">Toda la comida ha sido consumida en ${finalTime} el ${gameDate}.</p>`;
            gridElement.style.border = 'none'; 
            gridElement.style.boxShadow = 'none'; 

            logMessage(`¡Juego Terminado! Toda la comida ha sido consumida en ${finalTime} el ${gameDate}.`, 'yellow');
            document.getElementById('startButton').disabled = true;
            document.getElementById('stepButton').disabled = true;
        }
    } catch (simError) {
...
function displayGameHistory() {
    ...
    if (gameTimes.length === 0) {
        ...
        gameTimes.slice().reverse().forEach(entry => {
...
document.getElementById('startButton').addEventListener('click', () => {
    ...
    if (messageLogElement) {
        ...
        logMessage("Simulación iniciada. ¡Buena suerte, agente!");
    }
